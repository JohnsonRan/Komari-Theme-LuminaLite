...
function HomeBrand({ siteName }: { siteName: string }) {
  ...
  return (
    <header className="home-brand" aria-label="站点名称">
      ...
      <span className="home-brand-versions">
        {version?.version && (
          <span
            className="home-brand-version home-brand-version--server"
            title={`Komari ${version.version} (${version.hash.slice(0, 7)})`}
          >
            v{version.version}
          </span>
        )}
        <span className="home-brand-version home-brand-version--theme" title={`LuminaLite ${__THEME_VERSION__}`}>
          Lite {__THEME_VERSION__}
        </span>
      </span>
    </header>
  );
